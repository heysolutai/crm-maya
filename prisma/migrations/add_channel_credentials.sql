...
-- 3) Backfill: extrai credenciais ja existentes em whatsapp_instances.channel_config
--    Pega a instancia mais antiga de cada (empresa, canal) como fonte da credencial.
WITH src AS (
...
SELECT
  ...
  s.channel_type,
  s.api_url,
  s.channel_config->>'serverApiKey'
  FROM src s
 WHERE NOT EXISTS (
   SELECT 1 FROM channel_credentials c
    WHERE c.company_id = s.company_id
      AND c.channel_type = s.channel_type
 );
